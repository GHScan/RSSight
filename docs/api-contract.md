...
- `POST /api/feeds/{feedId}/articles/{articleId}/move` — body `{ "target_feed_id" }`; moves the entire article directory (including summaries) from one virtual feed to another. Updates `article.json` `feed_id` and any matching read-later reference. Returns 204. 400 `SAME_SOURCE_AND_TARGET` if `target_feed_id` equals `feedId`; 400 `NOT_VIRTUAL_FEED` if either feed is not virtual; 404 `FEED_NOT_FOUND` / `ARTICLE_NOT_FOUND`; 409 `ARTICLE_EXISTS_IN_TARGET` if the target feed already has an article directory with the same `articleId`.
...
- `POST /api/feeds/{feedId}/refresh` — manually re-fetch RSS for this feed and persist articles. Returns 204 on success. 404 if feed not found; 400 with `NOT_RSS_FEED` if feed is virtual or has no URL; 502 with `REFRESH_FAILED` on fetch failure.
...
- `GET /api/read-later` — returns list of read-later items: `[{ "feed_id", "article_id", "added_at" (ISO), "title", "published" (ISO) }, ...]` in newest-added-first order. The `title` field is the display title (S068: prefer `title_trans` when non-empty). The `published` field is the article's published date for date/color marker in the UI (S069). Entries whose article no longer exists are omitted.
